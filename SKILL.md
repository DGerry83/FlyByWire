---
name: flybywire
description: Protocol-driven workflow router for software engineering tasks. Classifies a request (bug, feature, research, refactor, cleanup, plan execution, changelog, UX validation, design spec, new project, artifact cleanup) and runs the matching phase-based protocol under shared core rules.
whenToUse: When the user wants a structured, phase-based workflow for fixing a bug, planning or building a feature, researching a technology or codebase, refactoring, behavior-preserving cleanup, executing a pre-written plan, generating a changelog from commits, validating UX/settings labels against behavior, refining an idea into a design spec, bootstrapping a new project, or reorganizing accumulated artifacts.
---
