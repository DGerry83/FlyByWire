...
1. **Session artifacts** (REFACTOR_MAP, REFACTOR_DESIGN, GATES, PROGRESS_LOG, REFACTOR_AUDIT):
   - Place in `[notes\active\YYYY-MM-DD_[Type]_[Description]\]`
   - Example: `[notes\active\2026-06-09_Refactor_GameUISplit\]`
2. **Consolidated reference docs** (architecture overviews, debugging guides, build instructions):
   ...
   - Example: `[notes\knowledge\StoreSlicePatterns.md]`
3. **Plans, backlogs, and research** (bug lists, implementation roadmaps, research findings):
   - Place in `[notes\plans\[descriptive-name].md]` or subfolder
   - Example: `[notes\plans\REFACTORBACKLOG-2026-06.md]`
...
**Frozen means frozen.** After `GATES.md` is written and the user approves moving into Phase 2, do not modify it. Any post-freeze modification — including retroactive wording changes, scope narrowing, or criterion relaxation — renders the affected gate `INVALID` and is an automatic audit failure.
...
**Sub-Agent Prompt Template**:
   ...
   Before writing or moving any code, compare the provided REFACTOR_MAP.md and REFACTOR_DESIGN.md against the actual project files.
   1. List every disagreement, contradiction, or ambiguity you find between the contract and reality. Cite real file paths, function names, and line numbers where applicable.
   ...
   1. **Read Project Context**:
      - Read the project onboarding docs (AGENTS.md, README.md) for build commands
      - Read REFACTOR_MAP.md, REFACTOR_DESIGN.md, and GATES.md completely
   ...
   2. **Environment Validation**:
      ...
      - Verify build system access: Can you read project config and build scripts?
      - Test compile: Perform a clean build before any changes to establish baseline
      - **Shell Syntax**: Use the chaining operator native to the detected shell (`;` in PowerShell, `&&` in bash)
   
   3. **Acknowledge**: Reply "Onboarding complete. Baseline build verified. Ready."
...
1. Save all state to disk (maps, designs, gates, progress logs, audit reports)
