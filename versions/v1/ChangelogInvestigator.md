...
Place the generated changelog in the active session folder for the release or comparison being analyzed:

```text
notes\active\YYYY-MM-DD_[Feature|Bug|Refactor|Research|PlanName]_[Description]\CHANGELOG.md
```

If the changelog is part of a planned release or research effort, you may also place a copy under `notes\plans\[release-or-research-name]\`. Update `notes\indices\master_index.md` with the session summary and a link to the changelog when the session closes. See [`.\CORE_PROTOCOLS.md`](.\CORE_PROTOCOLS.md) for the full taxonomy, session naming convention, and archive lifecycle.
...
```markdown
## Changelog: `<BASE>` → `HEAD` (`<CURRENT_BRANCH>`)
...
### Merge Activity (if any)
- ...

### Needs Human Review
- List anything flagged as suspicious, unusually large, or ambiguous in intent.
```
