...
Place all UX audit artifacts in a dated active session folder:

```text
notes\active\YYYY-MM-DD_[Feature|Bug|Refactor|Research|PlanName]_[Description]\
```
...
Use the session naming convention from [`.\CORE_PROTOCOLS.md`](.\CORE_PROTOCOLS.md): `YYYY-MM-DD_[Feature|Bug|Refactor|Research|PlanName]_[Description]`. When the audit closes, update `notes\indices\master_index.md` with the session summary and status, and extract any reusable label patterns to `notes\knowledge\`.
