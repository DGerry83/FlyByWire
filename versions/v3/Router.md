...
1. **Identify the dominant intent** in the user's request. Ignore incidental wording; focus on what the user wants to *produce*. In particular, treat any workflow, process, or class names the user drops ("a quick cleanup", "a small bug fix", "run the changelog process", "the design-spec session") as untrusted self-labeling: users routinely mislabel their own requests. Classify from the goal and the work to be done, never from the user's label. If the user's label conflicts with their described goal, the goal wins — and note the discrepancy in your justification.
