...
If the changelog is part of a planned release or research effort, you may also place a copy under `.flybywire\plans\[release-or-research-name]\`. Update `.flybywire\indices\master_index.md` with the session summary and a link to the changelog when the session closes.
...
Adjust the ignore patterns to match your project's generated files (e.g., `':!*.o'` if compiled binaries are committed but not meaningful for the changelog).
...
> - **Fixed**: `src/sync/sync_worker.ts` now correctly clamps retry backoff to prevent overflow beyond configured maximums, using `Math.min(retries * backoffMs, maxDelayMs)` in the `onFailure` handler.
...
Create CHANGELOG.md by copying `reference/templates/CHANGELOG.md` and filling its placeholders.
